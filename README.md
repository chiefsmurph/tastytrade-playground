...
- `BOT_CASH_MARGIN_YES_DOWN_PCT` (position gate: cross-account ask-return must be below this percentage to count as a "cross-account YES" signal; default `10`)
- `BOT_MARGIN_CROSS_ACCOUNT_THRESHOLD_MULTIPLIER` (position gate: multiplier on the cross-account YES threshold when running for the margin account — requires the cash position to be down proportionally more than the cash account requires of margin; default `2`)
