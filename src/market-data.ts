...
export async function getBidAskForSymbol(symbol: string, timeoutMs = 3000): Promise<{ bid?: number; ask?: number } | null> {
  try {
    await tastytradeApi.quoteStreamer.connect();

    const candidates = [symbol, `.${symbol}`, symbol.replace('/', '.' )].filter(Boolean);

    return await new Promise((resolve) => {
      const timer = setTimeout(() => cleanupAndResolve(null), timeoutMs);

      function cleanupAndResolve(result: any) {
        try { tastytradeApi.quoteStreamer.unsubscribe(candidates); } catch {}
      ...
      const removeListener = tastytradeApi.quoteStreamer.addEventListener((events: any[]) => {
        ...
        for (const ev of arr) {
          ...
          if (!candidates.includes(evSym) && !candidates.includes(norm) && !candidates.includes(evSym.replace(/^\./, ''))) continue;
      ...
      try { tastytradeApi.quoteStreamer.subscribe(candidates); } catch (e) {
