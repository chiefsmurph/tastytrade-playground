...
export default async function everyFourMinutes() {
  ...
  const accountBalances =
    await tastytradeApi.balancesAndPositionsService.getAccountBalanceValues(accountNumber);
  console.log("Current account balances:", JSON.stringify(accountBalances, null, 2));
